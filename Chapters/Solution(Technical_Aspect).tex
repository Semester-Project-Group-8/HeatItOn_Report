...
% -------------------------------------------------------------
\noindent This section will outline the proccess of creation of the project, from project description to final solution, also describing the technical architecture. The solution started off with going through the provided Project Description. From here important parts were selected and wrote into a compact version. This compact version was suitable for conducting the verb/noun analysis where class and method candidates emerged. The class candidates are able to be transformed into CRC cards outlining the responsabilities and collaborators of each class. The CRC cards are used to create UML diagrams(Class,Package,Activity and Sequence). The different diagrams later on were used for implementation.
...
The first one is the Frontend package, containing the Asset, Result and Source Managers. This is the UI of the system, with what the user will interact. The Frontend is following the Model-View-ViewModell pattern, layering the different parts of each manager. This package connects with the next package through clients.
...
This diagram shows a very simplified relationship with the most important parts of the overall system, giving an understanding of the overall workings and layers. Allthough its very simple, it is a good starting point for understanding the entire archtecture.
...
Importing is initiated by the user interacting with the Source Manager UI. The Source Manager then will try to find the requested csv file. If the file is found the Source Manager will try to open and read the data in the FrontEnd. After a successful opening and reading the data will be transformed into proper Source models and will be sent over through the Backend to the Database for storage, notifying the user. If the file is not found or if the file could not be opened or read properly, like if the csv is not structured properly, the importing process failes and the user is notified. The activity is concluded by notifying the user of either a successful or failed import, the former is visible by a new data set apearing in the Source Manager.
...
The Source Manager and Sources are the other crucial part of the system, which without, the optimization process could not be run.
...
example. Deletion is crucial the the proper workings of the system.
