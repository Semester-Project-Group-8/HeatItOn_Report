...
\subsection{Final Version}
The final backend version extends the original layered ASP.NET Core architecture with additional runtime functionality and tighter system integration. Two supporting layers were formalized and integrated to improve maintainability and consistency: a centralized exception-handling middleware and a stronger `Interfaces` layer.

These additions work together with the existing layers as follows: controllers accept HTTP requests and delegate work to services; services implement business logic and interact with the data layer (EF `BackendDbContext`); both controllers and services rely on the `Interfaces` layer for abstraction and dependency injection; the `ExceptionHandlingMiddleware` sits early in the request pipeline, catching exceptions from controllers and services, logging them, mapping domain errors to HTTP status codes, and returning compact JSON error responses. As a result, many local try/catch blocks were removed and method implementations became simpler.

\begin{itemize}
    \item \textbf{Exception handling}: Centralized via a custom middleware (\texttt{ExceptionHandlingMiddleware}) that intercepts exceptions thrown by controllers and services, logs them, maps domain errors to appropriate HTTP status codes, and returns a compact JSON payload (for example \texttt{\{ "error": "<message>", "stackTrace": "<stackTrace>" \}}) with \texttt{Content-Type: application/json}. This provides consistent, machine-readable error responses for clients and prevents raw error details from leaking.
    ...
    \begin{itemize}
        ...
        \end{itemize}
    \end{itemize}
    \item \textbf{Controllers}: Handle request/response concerns and remain thin — delegating business rules to services and relying on interfaces for testability.
    \item \textbf{Services}: Contain core business logic, validate inputs, interact with the data layer, and throw well-defined domain exceptions when needed. They do not implement HTTP concerns or repeated try/catch logic anymore.
...
\subsection{Final Version}
The recent database updates were relatively minor and focused on strengthening data integrity. We refined several model-level constraints using data-annotation attributes (e.g., Required, Range, MaxLength) to ensure that invalid or inconsistent values cannot be stored. Additionally, a unique constraint was introduced for the Source entity through the DbContext configuration, preventing duplicate source entries from being created. These adjustments improve validation and consistency across the system without modifying the core schema.