...
The final version represents a substantial advancement in both functionality and user experience when compared to the midterm implementation. The primary objective of this iteration was to refine usability, improve visual clarity, and introduce additional analytical capabilities while maintaining a consistent and scalable structure. 

A significant improvement is the transition from a manual data refresh system to a dynamic, responsive interface. In contrast to the midterm version, where a dedicated refresh button was required to update data, the final implementation ensures that all lists and views update automatically in response to changes in the database. This enhancement improves workflow efficiency and eliminates unnecessary user interaction.

The visual design of the interface has been considerably enhanced. The final version adopts a modern, structured design language with consistent color coding, clearer typography, and improved spacing. Asset cards have been redesigned to display key information in a concise and readable format, including maximum heat, production cost, CO\textsubscript{2} emissions, and electricity generation or consumption. Additionally, asset categories such as gas, oil, electric, and motor are visually distinguished through icons and labels, allowing for faster recognition and comparison.
...
Additional control elements have been introduced, including a dedicated “Manager” button and a more prominent “Optimize” action button. The Manager section serves as a centralized interface for creating, importing, and exporting assets. These improvements contribute to a clearer workflow hierarchy, guiding the user from configuration to execution in a more structured manner.
...
The tabular data display has also been improved with better formatting, clearer column labeling, and pagination support for large datasets. The addition of pagination ensures that even extensive datasets remain accessible and manageable.
...
\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{Images/backend-file-structure.png}
...
\begin{itemize}
    ...
    \item \textbf{Interfaces}: Define abstraction layers for both controllers and services. Generic interfaces such as \texttt{IController<TCreate, TUpdate>} and \texttt{IService<T>} enforce consistent method definitions and reduce code duplication.
...
The backend exposes a RESTful API through controllers that implement a generic controller interface. The interface defines standard CRUD operations:
...
All controller actions are implemented asynchronously using \texttt{Task<IActionResult>}, allowing efficient handling of concurrent requests. Controllers delegate all processing tasks to the corresponding service layer, ensuring minimal logic within the API layer.
